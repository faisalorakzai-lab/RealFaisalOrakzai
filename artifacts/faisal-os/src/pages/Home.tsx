...
function PhilWord({ word, index, total, scrollProgress }: { word: string; index: number; total: number; scrollProgress: ReturnType<typeof useScroll>["scrollYProgress"] }) {
  const start = Math.min(index / total, 0.95);
  const end = Math.min((index + 2) / total, 1);
  const color = useTransform(scrollProgress, [start, end], ["rgba(255,255,255,0.22)", "rgba(255,255,255,0.92)"]);
