        ...
        {/* ── EXPANDED PANEL: Live Core info ── */}
              ...
              <div style={{ display: "flex", animation: "ticker 50s linear infinite", width: "max-content", alignItems: "center" }}>
                {[...UNDER_DEV, ...LIVE_CORES.map(c => ({ id: c.id, name: c.name, ticker: c.ticker, tag: c.tag, statusColor: c.statusColor, desc: "" })), ...UNDER_DEV, ...LIVE_CORES.map(c => ({ id: c.id, name: c.name, ticker: c.ticker, tag: c.tag, statusColor: c.statusColor, desc: "" }))].map((v, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "0 28px", borderRight: "1px solid rgba(255,255,255,0.05)", flexShrink: 0 }}>
                      ...
                      <span style={{ fontFamily: "monospace", fontSize: "10px", fontWeight: 800, color: v.statusColor, whiteSpace: "nowrap", letterSpacing: "0.05em" }}>{v.ticker}</span>
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 20px" }}>
            ...
            <div style={{ position: "relative", overflow: "hidden", padding: "4px 0" }}>
                <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: "80px", background: "linear-gradient(to right, #000, transparent)", zIndex: 2, pointerEvents: "none" }} />
                <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "80px", background: "linear-gradient(to left, #000, transparent)", zIndex: 2, pointerEvents: "none" }} />
                <div style={{ display: "flex", animation: "ticker 45s linear infinite", width: "max-content", alignItems: "stretch", gap: "1px" }}>
                  {[...UNDER_DEV, ...UNDER_DEV].map((venture, i) => (
                    <motion.div key={i}
                      whileHover={{ scale: 1.02 }}
                      onClick={() => VENTURE_DETAILS[venture.id] ? setActiveVenture(activeVenture === venture.id ? null : venture.id) : undefined}
                      style={{ width: "260px", background: activeVenture === venture.id ? `${venture.statusColor}10` : "#0a0a0a", padding: "18px 20px", display: "flex", gap: "14px", alignItems: "flex-start", position: "relative", overflow: "hidden", cursor: VENTURE_DETAILS[venture.id] ? "pointer" : "default", border: `1px solid ${activeVenture === venture.id ? venture.statusColor + "40" : "rgba(255,255,255,0.04)"}`, flexShrink: 0 }}
                    >
                      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: `linear-gradient(90deg, ${venture.statusColor}80, transparent)` }} />
                      <div style={{ width: "44px", height: "44px", borderRadius: "50%", border: `1.5px solid ${venture.statusColor}30`, overflow: "hidden", flexShrink: 0, background: "#050505", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {venture.logo ? (
                          <img src={venture.logo} alt={venture.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                        ) : (
                          <span style={{ fontFamily: "monospace", fontSize: "9px", fontWeight: 800, color: venture.statusColor }}>{venture.ticker}</span>
                        )}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: "monospace", fontSize: "7px", color: "rgba(255,255,255,0.2)", letterSpacing: "0.2em", marginBottom: "3px" }}>{venture.tag}</div>
                        <div style={{ fontSize: "13px", fontWeight: 700, color: "#fff", marginBottom: "4px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{venture.name}</div>
                        <div style={{ fontFamily: "monospace", fontSize: "7px", color: venture.statusColor, letterSpacing: "0.2em", display: "flex", alignItems: "center", gap: "5px" }}>
                          <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: venture.statusColor, display: "inline-block" }} />
                          {VENTURE_DETAILS[venture.id] ? "TAP TO EXPAND" : "BUILDING"}
                        </div>
                      </div>
                    </motion.div>
                  ))}
    ...
    const ORG_DETAILS: Record<string, {
      ...
      "pihro": {
          ...
          uniqueFeatures: [
            ...
            "Dual-leadership structure: national chairman + Karachi city chairman for effective local advocacy",
            "International mandate — engages with global human rights bodies on Pakistan-specific issues",
            "Legal support and documentation services for victims of rights violations across Pakistan",
            "Community-level outreach and awareness programs across Karachi and nationally",
            "Orakzai lineage-driven ethos of justice, sovereignty, and unwavering service to the people",
          ],
          benefits: [
            ...
            { label: "Community Empowerment", desc: "Educates communities on their legal rights and equips them with tools to advocate effectively for themselves." },
          ...
          websiteUrl: "https://pihro.org",
          socialUsername: "pihropakistan",
        },
        "son-of-orakzai": {
          title: "Digital Embassy for the Orakzai Community — Unity, Service & Empowerment (Est. 2015)",
          bio: "Son Of Orakzai (SOOORG) is a registered NGO in Pakistan acting as a 'Digital Embassy' for the Orakzai community. Founded in 2015 by Malak Speen Gul Orakzai and currently chaired by Faisal Orakzai, it is a movement born from a deep love for the homeland — striving to build a future where every member of the Orakzai community is empowered, educated, and prosperous. It bridges tradition and modern progress: a digital homeland where every member is seen, heard, and empowered.",
          description: "Son Of Orakzai is a premium global NGO platform dedicated to uplifting the Orakzai district in Khyber Pakhtunkhwa, Pakistan. Operating with core values of Unity, Service, and Empowerment, it maintains a National HQ in Karachi and regional offices across Pakistan and internationally. Key initiatives: Digital Skill Lab (AI & tech bootcamps), Sehat-e-Orakzai (free medical camps & telemedicine), Jirga-e-Naujawan (online complaint resolution modernising the traditional Jirga), Talent Hunt (annual sports & academic festivals), and Imdad-e-Bahan (emergency relief & widow support).",
          leadership: [
            { role: "Founder", name: "Malak Speen Gul Orakzai" },
            { role: "Chairman", name: "Faisal Orakzai" },
          ],
          uniqueFeatures: [
            "Digital Embassy Concept — uniquely a digital homeland blending Orakzai tradition with modern digital empowerment",
            "Jirga-e-Naujawan — modernising the traditional Jirga into an online complaint & resolution portal for youth and elders",
            "Digital Skill Lab — 6-month intensive bootcamps, freelancing mentorship, and AI & tech workshops for Orakzai youth",
            "Sehat-e-Orakzai — free medical camps, telemedicine portal, and maternal health support across the district",
            "Comprehensive multi-sector NGO: healthcare, emergency relief, digital skills, governance, and cultural preservation in one",
            "Globally structured with National HQ in Karachi — a scalable model for rural community development worldwide",
          ],
          benefits: [
            { label: "Global Integration", desc: "Training youth in AI, Web Development, and Freelancing integrates a previously marginalised community into the global digital economy." },
            { label: "Rural Development Model", desc: "Scalable model for how remote traditional communities can leverage technology for self-empowerment, healthcare access, and transparent governance." },
            { label: "Cultural Preservation", desc: "Demonstrates how communities can embrace global progress without losing their Orakzai cultural identity and traditional roots." },
            { label: "Healthcare Access", desc: "Free medical camps and telemedicine portal bring quality healthcare to remote Orakzai district communities who lack access." },
            { label: "Community Welfare", desc: "Emergency relief funds, widow support, and educational scholarships ensure no Orakzai family is left behind." },
          ],
          roadmap: [
            { milestone: "2015 — Foundation", desc: "Established with a vision to unite the global Orakzai community around empowerment and progress." },
            { milestone: "2018 — Education Initiative", desc: "Launched the first scholarship program for deserving students from the Orakzai district." },
            { milestone: "2020 — Health Camps", desc: "Started regular free medical camps across the Orakzai district for underserved communities." },
            { milestone: "2023 — Digital Era", desc: "Launched the Digital Skill Lab empowering youth with AI, freelancing, and modern tech skills." },
            { milestone: "Ongoing — Global Expansion", desc: "Expanding digital initiatives internationally, preparing the next generation, ensuring no family is left behind." },
          ],
          websiteUrl: "https://sooog.org",
          socialUsername: "sooogpakistan",
        }
    };
                ...
                >
                  <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 20px 20px" }}>
                    <div style={{ border: `1px solid ${org.statusColor}25`, borderTop: `3px solid ${org.statusColor}`, background: `linear-gradient(180deg, ${org.statusColor}06 0%, transparent 40%)`, padding: "32px" }}>
                      ...
                      {/* Website + Social */}
                      {(d.websiteUrl || d.socialUsername) && (
                        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginTop: "24px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                          {d.websiteUrl && <a href={d.websiteUrl} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 16px", border: `1px solid ${org.statusColor}40`, background: `${org.statusColor}08`, textDecoration: "none" }}>
                            <span style={{ fontFamily: "monospace", fontSize: "10px", color: org.statusColor, letterSpacing: "0.1em" }}>🌐 {d.websiteUrl.replace("https://", "")}</span>
                          </a>}
                          {d.socialUsername && <a href={`https://twitter.com/${d.socialUsername}`} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 16px", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)", textDecoration: "none" }}>
                            <span style={{ fontFamily: "monospace", fontSize: "10px", color: "#fff" }}>𝕏 @{d.socialUsername}</span>
                          </a>}
                        </div>
                      )}
                      {/* Roadmap */}
                      {d.roadmap && d.roadmap.length > 0 && (
                        <div style={{ marginTop: "24px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                          <div style={{ fontFamily: "monospace", fontSize: "9px", color: org.statusColor, letterSpacing: "0.3em", marginBottom: "14px" }}>// TIMELINE & ROADMAP</div>
                          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                            {d.roadmap.map((r, i) => (
                              <div key={i} style={{ display: "flex", gap: "14px", alignItems: "flex-start", padding: "10px 14px", border: "1px solid rgba(255,255,255,0.04)", background: "rgba(255,255,255,0.01)" }}>
                                <span style={{ fontFamily: "monospace", fontSize: "9px", color: org.statusColor, whiteSpace: "nowrap", flexShrink: 0, marginTop: "2px" }}>▸ {r.milestone}</span>
                                <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", lineHeight: 1.5 }}>{r.desc}</span>
                              </div>
                            ))}
                          </div>
                        </div>
