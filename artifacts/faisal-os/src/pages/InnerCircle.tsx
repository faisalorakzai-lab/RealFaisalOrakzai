 ...
 * Phase 4: Sovereign Cryptographic Passport Card Generator (HTML/CSS + Signature Canvas + Export)
...
import {
  ...
  Heart, Clock, Radio, ChevronRight, ChevronDown,
...
───────────────────────────────────────── */
function useSignatureCanvas(canvasRef: React.RefObject<HTMLCanvasElement | null>, strokeColor: string) {
...
function Tier1Card({ userName, memberId, userPhotoUrl, userSignatureUrl, founderSignatureUrl, issueDate, expiryDate, qrDataUrl, logoUrl }: CardProps) {
  return (
    <div
      className="relative overflow-hidden text-white font-mono select-none"
      style={{
        width: 540, height: 320,
        background: "radial-gradient(ellipse at top left, rgba(8,12,20,1) 0%, rgb(2,2,5) 100%)",
        border: "1px solid rgba(6,182,212,0.35)",
        borderRadius: 24,
        boxShadow: "0 0 40px rgba(6,182,212,0.12), inset 0 0 60px rgba(6,182,212,0.03)",
        padding: 28,
    ...
    >
      ...
      <div style={{
        ...
        backgroundSize: "20px 20px", borderRadius: 24,
      }} />
      ...
      <div style={{
        ...
        <span style={{ fontSize: 11, letterSpacing: "0.3em", color: "#06b6d4", fontWeight: 900, transform: "rotate(-15deg)", whiteSpace: "nowrap", textAlign: "center" }}>
          OFFICIAL INTEGRITY · ORAKZAI GROUP · OFFICIAL INTEGRITY · ORAKZAI GROUP
        </span>
      </div>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative", zIndex: 10 }}>
        <div>
          <div style={{ fontSize: 10, color: "#06b6d4", fontWeight: 900, letterSpacing: "0.2em", textTransform: "uppercase" }}>OkzByte Hub</div>
          <div style={{ fontSize: 8, color: "rgba(255,255,255,0.35)", letterSpacing: "0.12em", marginTop: 2 }}>Silicon Valley Network Node // Tier 01</div>
        </div>
        <div style={{
          width: 48, height: 48, background: "rgba(15,15,20,0.9)", padding: 6,
          borderRadius: 14, border: "1px solid rgba(6,182,212,0.25)", marginTop: -6, marginRight: -6,
          boxShadow: "0 0 20px rgba(6,182,212,0.1)",
        }}>
          <img src={logoUrl} style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 0 6px rgba(6,182,212,0.5))" }} />
        </div>
      </div>
      {/* Main content */}
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 20, position: "relative", zIndex: 10 }}>
        {/* Photo */}
        <div style={{
          width: 96, height: 96, borderRadius: 14, border: "1.5px solid rgba(6,182,212,0.45)",
          background: "rgba(15,15,25,0.9)", overflow: "hidden", flexShrink: 0,
          ...
          {userPhotoUrl ? (
            ...
            <span style={{ fontSize: 28, fontWeight: 900, color: "#06b6d4" }}>{(userName || "?")[0].toUpperCase()}</span>
          )}
        </div>
        {/* Name & badge */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 900, color: "#f1f5f9", letterSpacing: "0.08em", textTransform: "uppercase", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {userName || "IDENTITY NAME"}
          </div>
          <div style={{
            marginTop: 6, display: "inline-block",
            fontSize: 9, color: "#06b6d4", fontWeight: 700, letterSpacing: "0.15em",
        ...
        {/* QR code */}
        {qrDataUrl && (
          <div style={{ flexShrink: 0, width: 68, height: 68, background: "white", borderRadius: 6, padding: 4, boxShadow: "0 0 10px rgba(6,182,212,0.2)" }}>
            <img src={qrDataUrl} style={{ width: "100%", height: "100%", imageRendering: "pixelated" }} />
          </div>
        )}
      </div>
      {/* Bottom section */}
      <div style={{ position: "absolute", bottom: 22, left: 28, right: 28, display: "flex", justifyContent: "space-between", alignItems: "flex-end", zIndex: 10 }}>
        {/* Founder signature */}
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <img src={founderSignatureUrl} style={{ height: 32, maxWidth: 110, objectFit: "contain", filter: "invert(1) brightness(0.7) sepia(1) hue-rotate(160deg) saturate(3)" }} />
          <div style={{ fontSize: 7, color: "rgba(255,255,255,0.35)", fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase" }}>ISSUING AUTHORITY: F. ORAKZAI</div>
          <div style={{ display: "flex", gap: 12, fontSize: 7, color: "rgba(255,255,255,0.3)", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>
            <span>ISSUE: <span style={{ color: "rgba(255,255,255,0.55)" }}>{issueDate}</span></span>
            <span>EXPIRY: <span style={{ color: "rgba(255,255,255,0.55)" }}>{expiryDate}</span></span>
          </div>
        </div>
        {/* User signature */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          <div style={{
            height: 38, width: 110, position: "relative", background: "rgba(15,15,25,0.3)",
            border: "1px solid rgba(255,255,255,0.08)", borderRadius: 4,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            {userSignatureUrl && (
              <img src={userSignatureUrl} style={{ height: 30, maxWidth: 100, objectFit: "contain", filter: "brightness(1.2) hue-rotate(140deg)" }} />
            )}
            <span style={{ position: "absolute", bottom: 2, right: 4, fontSize: 6, color: "rgba(255,255,255,0.25)", letterSpacing: "0.05em" }}>MEMBER SIGNATURE</span>
          </div>
          {/* Microchip */}
          <div style={{
            width: 40, height: 30, borderRadius: 4,
            background: "linear-gradient(135deg, #1e2030 0%, #2a2d45 50%, #1a1d2e 100%)",
            border: "1px solid rgba(6,182,212,0.3)", position: "relative", overflow: "hidden",
          }}>
            <div style={{ position: "absolute", top: "33%", left: 0, right: 0, height: 1, background: "rgba(6,182,212,0.25)" }} />
            <div style={{ position: "absolute", top: "66%", left: 0, right: 0, height: 1, background: "rgba(6,182,212,0.25)" }} />
            <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 1, background: "rgba(6,182,212,0.25)" }} />
          </div>
        </div>
      </div>
    </div>
...
function Tier2Card({ userName, memberId, userPhotoUrl, userSignatureUrl, founderSignatureUrl, issueDate, expiryDate, qrDataUrl, logoUrl }: CardProps) {
  return (
    <div
      className="relative overflow-hidden text-white font-mono select-none"
      style={{
        width: 540, height: 320,
        background: "radial-gradient(ellipse at top right, rgba(20,14,5,1) 0%, rgb(5,5,5) 100%)",
        border: "1px solid rgba(245,158,11,0.35)",
        borderRadius: 24,
        boxShadow: "0 0 45px rgba(245,158,11,0.12), inset 0 0 60px rgba(245,158,11,0.03)",
        padding: 28,
        fontFamily: "monospace",
      }}
    >
      ...
      <div style={{
        ...
        backgroundSize: "18px 18px", borderRadius: 24,
      }} />
      {/* Holographic watermark */}
      <div style={{
        position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center",
        ...
        <span style={{ fontSize: 11, letterSpacing: "0.3em", color: "#f59e0b", fontWeight: 900, transform: "rotate(-15deg)", whiteSpace: "nowrap", textAlign: "center" }}>
          OFFICIAL INTEGRITY · ORAKZAI GROUP · OFFICIAL INTEGRITY · ORAKZAI GROUP
        </span>
      </div>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative", zIndex: 10 }}>
        <div>
          <div style={{ fontSize: 10, color: "#f59e0b", fontWeight: 900, letterSpacing: "0.2em", textTransform: "uppercase" }}>OkzByte Hub</div>
          <div style={{ fontSize: 8, color: "rgba(255,255,255,0.4)", letterSpacing: "0.12em", marginTop: 2 }}>Silicon Valley Network Node // Tier 02</div>
        </div>
        <div style={{
          width: 48, height: 48, background: "rgba(15,12,5,0.9)", padding: 6,
          borderRadius: 14, border: "1px solid rgba(245,158,11,0.3)", marginTop: -6, marginRight: -6,
          boxShadow: "0 0 20px rgba(245,158,11,0.12)",
        }}>
          <img src={logoUrl} style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 0 6px rgba(245,158,11,0.5))" }} />
        </div>
      </div>
      {/* Main content */}
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 20, position: "relative", zIndex: 10 }}>
        <div style={{
          width: 96, height: 96, borderRadius: 14, border: "1.5px solid rgba(245,158,11,0.45)",
          background: "rgba(20,15,5,0.9)", overflow: "hidden", flexShrink: 0,
          ...
          {userPhotoUrl ? (
            ...
            <span style={{ fontSize: 28, fontWeight: 900, color: "#f59e0b" }}>{(userName || "?")[0].toUpperCase()}</span>
          )}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 900, color: "#fef3c7", letterSpacing: "0.08em", textTransform: "uppercase", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {userName || "IDENTITY NAME"}
          </div>
          <div style={{
            marginTop: 6, display: "inline-block",
            fontSize: 9, color: "#f59e0b", fontWeight: 700, letterSpacing: "0.15em",
        ...
        {qrDataUrl && (
          <div style={{ flexShrink: 0, width: 68, height: 68, background: "white", borderRadius: 6, padding: 4, boxShadow: "0 0 10px rgba(245,158,11,0.2)" }}>
            <img src={qrDataUrl} style={{ width: "100%", height: "100%", imageRendering: "pixelated" }} />
          </div>
        )}
      </div>
      ...
      <div style={{ position: "absolute", bottom: 22, left: 28, right: 28, display: "flex", justifyContent: "space-between", alignItems: "flex-end", zIndex: 10 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <img src={founderSignatureUrl} style={{ height: 32, maxWidth: 110, objectFit: "contain", filter: "sepia(1) brightness(1.3) hue-rotate(5deg) saturate(4)" }} />
          <div style={{ fontSize: 7, color: "rgba(255,255,255,0.35)", fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase" }}>ISSUING AUTHORITY: F. ORAKZAI</div>
          <div style={{ display: "flex", gap: 12, fontSize: 7, color: "rgba(255,255,255,0.3)", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>
            <span>ISSUE: <span style={{ color: "rgba(255,255,255,0.55)" }}>{issueDate}</span></span>
            <span>EXPIRY: <span style={{ color: "rgba(255,255,255,0.55)" }}>{expiryDate}</span></span>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          <div style={{
            height: 38, width: 110, position: "relative", background: "rgba(20,15,5,0.3)",
            border: "1px solid rgba(255,255,255,0.08)", borderRadius: 4,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            {userSignatureUrl && (
              <img src={userSignatureUrl} style={{ height: 30, maxWidth: 100, objectFit: "contain", filter: "sepia(1) brightness(1.4) saturate(3) hue-rotate(15deg)" }} />
            )}
            <span style={{ position: "absolute", bottom: 2, right: 4, fontSize: 6, color: "rgba(255,255,255,0.25)", letterSpacing: "0.05em" }}>MEMBER SIGNATURE</span>
          </div>
          <div style={{
            width: 40, height: 30, borderRadius: 4,
            background: "linear-gradient(135deg, #78350f 0%, #92400e 50%, #713f12 100%)",
            border: "1px solid rgba(245,158,11,0.3)", position: "relative", overflow: "hidden",
          }}>
            <div style={{ position: "absolute", top: "33%", left: 0, right: 0, height: 1, background: "rgba(245,158,11,0.25)" }} />
            <div style={{ position: "absolute", top: "66%", left: 0, right: 0, height: 1, background: "rgba(245,158,11,0.25)" }} />
            <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 1, background: "rgba(245,158,11,0.25)" }} />
          </div>
        </div>
      </div>
    </div>
...
function Tier3Card({ userName, memberId, userPhotoUrl, userSignatureUrl, founderSignatureUrl, issueDate, expiryDate, qrDataUrl, logoUrl }: CardProps) {
  return (
    <div
      className="relative overflow-hidden text-white font-mono select-none"
      style={{
        width: 540, height: 320,
        background: "radial-gradient(ellipse at top right, rgba(20,20,20,1) 0%, rgb(0,0,0) 100%)",
        border: "1px solid rgba(200,200,200,0.5)",
        borderRadius: 24,
        boxShadow: "0 0 50px rgba(255,255,255,0.08), inset 0 0 60px rgba(255,255,255,0.02)",
        padding: 28,
        fontFamily: "monospace",
      }}
    >
      {/* Checkerboard bg */}
      <div style={{
        position: "absolute", inset: 0, opacity: 0.12, borderRadius: 24, overflow: "hidden",
        backgroundImage: "linear-gradient(45deg,#1f1f1f 25%,transparent 25%,transparent 75%,#1f1f1f 75%,#1f1f1f),linear-gradient(45deg,#1f1f1f 25%,transparent 25%,transparent 75%,#1f1f1f 75%,#1f1f1f)",
        backgroundSize: "24px 24px", backgroundPosition: "0 0,12px 12px",
      }} />
      {/* Holographic overlay */}
      <div style={{
        position: "absolute", inset: 0, borderRadius: 24, opacity: 0.08,
      ...
      <div style={{
        ...
        <span style={{ fontSize: 11, letterSpacing: "0.3em", color: "#ffffff", fontWeight: 900, transform: "rotate(-15deg)", whiteSpace: "nowrap", textAlign: "center" }}>
          OFFICIAL INTEGRITY · ORAKZAI GROUP · OFFICIAL INTEGRITY · ORAKZAI GROUP
        </span>
      </div>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative", zIndex: 10 }}>
        <div>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.85)", fontWeight: 900, letterSpacing: "0.2em", textTransform: "uppercase" }}>OkzByte Hub</div>
          <div style={{ fontSize: 8, color: "rgba(255,255,255,0.4)", letterSpacing: "0.12em", marginTop: 2 }}>Silicon Valley Network Node // Tier 03 Elite</div>
        </div>
        <div style={{
          width: 48, height: 48, background: "rgba(5,5,5,0.95)", padding: 6,
          borderRadius: 14, border: "1px solid rgba(200,200,200,0.4)", marginTop: -6, marginRight: -6,
          boxShadow: "0 0 20px rgba(255,255,255,0.06)",
        }}>
          <img src={logoUrl} style={{ width: "100%", height: "100%", objectFit: "contain", filter: "contrast(1.2) brightness(0.9)" }} />
        </div>
      </div>
      {/* Main content */}
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 20, position: "relative", zIndex: 10 }}>
        <div style={{
          width: 96, height: 96, borderRadius: 14, border: "1.5px solid rgba(200,200,200,0.6)",
          background: "rgba(15,15,15,0.95)", overflow: "hidden", flexShrink: 0,
          ...
          {userPhotoUrl ? (
            ...
            <span style={{ fontSize: 28, fontWeight: 900, color: "rgba(255,255,255,0.7)" }}>{(userName || "?")[0].toUpperCase()}</span>
          )}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 900, color: "#ffffff", letterSpacing: "0.08em", textTransform: "uppercase", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {userName || "IDENTITY NAME"}
          </div>
          <div style={{
            marginTop: 6, display: "inline-block",
            fontSize: 9, color: "rgba(255,255,255,0.8)", fontWeight: 700, letterSpacing: "0.15em",
            background: "rgba(255,255,255,0.06)", border: "1px solid rgba(200,200,200,0.3)",
            padding: "2px 8px", borderRadius: 4, boxShadow: "0 0 10px rgba(255,255,255,0.05)",
          }}>
            {memberId || "OKZ-2026-0000"}
          </div>
        </div>
        {qrDataUrl && (
          <div style={{ flexShrink: 0, width: 68, height: 68, background: "white", borderRadius: 6, padding: 4, boxShadow: "0 0 14px rgba(255,255,255,0.15)" }}>
            <img src={qrDataUrl} style={{ width: "100%", height: "100%", imageRendering: "pixelated" }} />
          </div>
        )}
      </div>
      ...
      <div style={{ position: "absolute", bottom: 22, left: 28, right: 28, display: "flex", justifyContent: "space-between", alignItems: "flex-end", zIndex: 10 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <img src={founderSignatureUrl} style={{ height: 32, maxWidth: 110, objectFit: "contain", filter: "invert(1) brightness(0.85)" }} />
          <div style={{ fontSize: 7, color: "rgba(255,255,255,0.35)", fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase" }}>ISSUING AUTHORITY: F. ORAKZAI</div>
          <div style={{ display: "flex", gap: 12, fontSize: 7, color: "rgba(255,255,255,0.3)", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>
            <span>ISSUE: <span style={{ color: "rgba(255,255,255,0.55)" }}>{issueDate}</span></span>
            <span>EXPIRY: <span style={{ color: "rgba(255,255,255,0.55)" }}>{expiryDate}</span></span>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          <div style={{
            height: 38, width: 110, position: "relative", background: "rgba(10,10,10,0.4)",
            border: "1px solid rgba(255,255,255,0.1)", borderRadius: 4,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            {userSignatureUrl && (
              <img src={userSignatureUrl} style={{ height: 30, maxWidth: 100, objectFit: "contain", filter: "brightness(1.5) contrast(1.2)" }} />
            )}
            <span style={{ position: "absolute", bottom: 2, right: 4, fontSize: 6, color: "rgba(255,255,255,0.25)", letterSpacing: "0.05em" }}>MEMBER SIGNATURE</span>
          </div>
          <div style={{
            width: 40, height: 30, borderRadius: 4,
            background: "linear-gradient(135deg, #1f1f1f 0%, #333 50%, #1a1a1a 100%)",
            border: "1px solid rgba(200,200,200,0.3)", position: "relative", overflow: "hidden",
          }}>
            <div style={{ position: "absolute", top: "33%", left: 0, right: 0, height: 1, background: "rgba(220,220,220,0.2)" }} />
            <div style={{ position: "absolute", top: "66%", left: 0, right: 0, height: 1, background: "rgba(220,220,220,0.2)" }} />
            <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 1, background: "rgba(220,220,220,0.2)" }} />
          </div>
        </div>
      </div>
    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={s.stat} className="flex items-center gap-4 border border-white/[0.06] px-5 py-4">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {benefits.map((b, i) => (
              ...
              >
                <div className="w-10 h-10 flex items-center justify-center mb-4 border border-white/10 group-hover:border-[#F3BA2F]/30 transition-colors" style={{ color: GOLD }}>
                  {b.icon}
                </div>
      ...
      <section className="py-24 px-6 border-t border-white/[0.05] text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
          ...
          <p className="text-white/35 text-sm mb-10 max-w-md mx-auto">
            Connect your wallet. Choose your tier. Receive your membership card. Access the hub. All on-chain. All sovereign.
          </p>
          <motion.button
            onClick={onNext}
            whileHover={{ scale: 1.03 }}
...
function WalletPhase({ onConnected }: { onConnected: (addr: string) => void }) {
  ...
  const connectMetaMask = async () => {
    ...
    try {
      ...
      try {
        ...
        if (err?.code === 4902) {
          await eth.request({
            ...
            params: [{
              chainId: CHAIN_ID_POLYGON, chainName: "Polygon Mainnet",
      ...
      if (err?.code !== 4001) { setErrMsg(err?.message || "Connection failed. Please try again."); setStatus("error"); }
      else setStatus("idle");
    }
  };

  const wallets = [
    { id: "metamask", name: "MetaMask", icon: "🦊", desc: "Browser extension wallet", action: connectMetaMask, real: true },
    { id: "walletconnect", name: "WalletConnect", icon: "🔵", desc: "Mobile & hardware wallets", action: () => { setErrMsg("Scan the QR from your mobile wallet app. (WalletConnect SDK coming soon.)"); setStatus("error"); }, real: false },
    { id: "coinbase", name: "Coinbase Wallet", icon: "🔷", desc: "Coinbase native wallet", action: () => { setErrMsg("Open Coinbase Wallet and use WalletConnect to connect. (Native SDK coming soon.)"); setStatus("error"); }, real: false },
  ];
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-6 pt-24 pb-16">
      <GoldParticles />
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative z-10 w-full max-w-sm">
        ...
        <p className="text-white/40 text-sm mb-8">Connect on Polygon Network. USDT / USDC accepted.</p>
        ...
        {busy ? (
          <div className="text-center py-12 border border-white/[0.07]">
            <motion.div className="w-12 h-12 border-2 border-[#F3BA2F]/20 border-t-[#F3BA2F] rounded-full mx-auto mb-4" animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} />
            <div className="text-white/60 text-sm font-mono">{status === "connecting" ? "Requesting account access..." : "Switching to Polygon..."}</div>
          </div>
        ) : (
          <div className="space-y-2">
            {wallets.map((w) => (
              <button key={w.id} onClick={w.action} className="w-full flex items-center gap-4 px-5 py-4 border border-white/[0.08] hover:border-[#F3BA2F]/30 hover:bg-[#F3BA2F]/5 transition-all text-left group">
        ...
        <AnimatePresence>
          {status === "error" && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mt-4 p-3 border border-red-500/30 bg-red-500/10 flex items-start gap-2">
...
───────────────────────────────────────── */
function TiersPhase({ walletAddr, isExpired, onBuy }: { walletAddr: string; isExpired: boolean; onBuy: (tier: Tier, txHash: string) => void }) {
  ...
  const executeBuy = async (tier: Tier) => {
    ...
    setBuying(tier); setTxStatus("pending"); setErrMsg("");
    try {
      ...
      const txHash = await eth.request({ method: "eth_sendTransaction", params: [{ from: walletAddr, to: USDT_POLYGON, data, gas: "0x186a0" }] }) as string;
      ...
      const err = e as { code?: number; message?: string };
      if (err?.code !== 4001) setErrMsg(err?.message || "Transaction failed. Please try again.");
      setTxStatus("error"); setBuying(null);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white pt-24 pb-20 px-6">
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-white mb-2">Choose Your<br /><span style={{ color: GOLD }}>Sovereignty Level</span></h1>
          <p className="text-white/40 text-sm mb-3 max-w-lg">Payment is made in USDT on Polygon. Access is granted immediately upon confirmation.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {([1, 2, 3] as Tier[]).map((t, i) => {
            const cfg = TIER_CFG[t]; const isPopular = t === 2; const isBuying = buying === t;
            return (
              <motion.div key={t} initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.1 }}
                className="relative border flex flex-col p-7"
                style={{ background: isPopular ? "#0a0a0a" : "black", borderColor: isPopular ? `${GOLD}50` : "rgba(255,255,255,0.08)", boxShadow: isPopular ? `0 0 50px ${GOLD}15` : undefined }}
              >
                {isPopular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 font-black font-mono text-[9px] tracking-widest uppercase text-black" style={{ background: GOLD }}>MOST POPULAR</div>}
                <div className="inline-flex items-center gap-1.5 self-start mb-5 px-2 py-1 font-mono text-[9px] tracking-widest uppercase border" style={{ color: cfg.accent, borderColor: `${cfg.accent}33` }}>
                  {cfg.emoji} {cfg.name}
                </div>
                ...
                {isBuying ? (
                  <div className="text-center py-4">
                    {txStatus === "pending" && (<><motion.div className="w-8 h-8 border-2 border-[#F3BA2F]/20 border-t-[#F3BA2F] rounded-full mx-auto mb-2" animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} /><div className="font-mono text-[10px] text-white/50">Awaiting MetaMask approval...</div></>)}
                    {txStatus === "confirming" && (<><motion.div className="w-8 h-8 border-2 border-emerald-400/20 border-t-emerald-400 rounded-full mx-auto mb-2" animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} /><div className="font-mono text-[10px] text-emerald-400">Confirming on Polygon...</div></>)}
                  </div>
                ) : (
                  <button onClick={() => executeBuy(t)} disabled={!!buying}
                    ...
                    style={isPopular ? { background: GOLD, color: "black" } : { border: `1px solid ${cfg.accent}44`, color: cfg.accent }}
                  >
                    <Wallet size={13} />BUY TICKET — {cfg.priceLabel} USDT
                  </button>
                )}
                {isBuying && txStatus === "error" && <p className="mt-2 text-red-400 text-[10px] font-mono text-center">{errMsg}</p>}
              </motion.div>
            );
          })}
        </div>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-center text-white/15 text-[10px] font-mono mt-8">
...
function CardPhase({ tier, memberId, txHash, onDone }: { tier: Tier; memberId: string; txHash: string; onDone: () => void }) {
  ...
  const cardProps: CardProps = {
    ...
    userSignatureUrl: sigDataUrl, founderSignatureUrl: FOUNDER_SIG_B64,
    issueDate, expiryDate, qrDataUrl, logoUrl: "/logos/okzbyte.png",
  };
    ...
    <div className="min-h-screen bg-black text-white pt-24 pb-20 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="text-3xl font-black text-white mb-1">Generate Your<br /><span style={{ color: GOLD }}>Sovereign Card</span></h1>
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="font-mono text-[9px] tracking-widest text-white/30 uppercase flex items-center gap-2">
                  <Pen size={10} />
                  Authorize Your Digital Signature
                </label>
                <div className="flex gap-2">
                  <button onClick={handleClearSig} className="flex items-center gap-1 font-mono text-[8px] text-white/25 hover:text-white/60 transition-colors px-2 py-1 border border-white/10">
                    <RotateCcw size={9} />CLEAR
                  </button>
                  <button
                    onClick={handleSigCapture}
              ...
              <div style={{ border: `1px solid ${cfg.accent}30`, borderRadius: 4, background: "rgba(0,0,0,0.6)", position: "relative" }}>
                ...
                {!hasDrawn && (
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
                    <span style={{ fontSize: 11, color: `${cfg.accent}30`, fontFamily: "monospace", letterSpacing: "0.15em" }}>DRAW YOUR SIGNATURE HERE</span>
                  </div>
                )}
              </div>
              {sigDataUrl && (
            ...
            <AnimatePresence>
              {exported && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  <button
                    onClick={onDone}
                    className="w-full flex items-center justify-center gap-2 py-4 font-bold text-sm tracking-wider font-mono text-black transition-all"
                    style={{ background: GOLD, boxShadow: `0 0 30px ${GOLD}50` }}
                  >
...
───────────────────────────────────────── */
function CommunityPhase({ tier, walletAddr, memberId, onExpired }: { tier: Tier; walletAddr: string; memberId: string; onExpired: () => void }) {
  const cfg = TIER_CFG[tier];
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (remaining <= 0) { LS.del("okz_wallet", "okz_tier", "okz_expires", "okz_tx"); onExpired(); return; }
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="border-b border-[#F3BA2F]/10 bg-black pt-20 pb-5 px-6 sticky top-0 z-40 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 border font-mono text-[9px] tracking-widest uppercase" style={{ color: cfg.accent, borderColor: `${cfg.accent}33` }}>
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: cfg.accent }} />
              {cfg.emoji} {cfg.name}
            </div>
            <div className="font-mono text-[9px] text-white/30 border border-white/10 px-2 py-1.5">{daysLeft}d left</div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="relative border p-5 overflow-hidden max-w-sm" style={{ background: `linear-gradient(135deg, ${cfg.cardBg[0]} 0%, ${cfg.cardBg[1]} 100%)`, borderColor: `${cfg.accent}40`, boxShadow: `0 0 30px ${cfg.accent}12` }}>
            <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: `linear-gradient(${cfg.accent} 1px, transparent 1px), linear-gradient(90deg, ${cfg.accent} 1px, transparent 1px)`, backgroundSize: "20px 20px" }} />
            <div className="relative z-10">
              ...
              <div className="flex justify-between">
                <div><div className="font-mono text-[8px] text-white/25 uppercase mb-0.5">Member ID</div><div className="font-mono text-xs" style={{ color: cfg.accent }}>{memberId}</div></div>
                <div className="text-right"><div className="font-mono text-[8px] text-white/25 uppercase mb-0.5">Expires</div><div className="font-mono text-xs text-white/60">{daysLeft} days</div></div>
              </div>
            </div>
            <motion.div className="absolute inset-0 pointer-events-none" style={{ border: `1px solid ${cfg.accent}` }} animate={{ opacity: [0.15, 0.5, 0.15] }} transition={{ duration: 3, repeat: Infinity }} />
          </div>
          <div className="mt-2 font-mono text-[8px] text-white/20">Wallet: {walletAddr.slice(0, 14)}...{walletAddr.slice(-6)}</div>
        </motion.div>
          ...
          <div className="space-y-2">
            {posts.map((p, i) => (
              <div key={i} className="border border-white/[0.06] p-4 hover:border-[#F3BA2F]/15 transition-colors">
                ...
                <button onClick={() => setLiked((prev) => ({ ...prev, [i]: !prev[i] }))} className="flex items-center gap-1.5 text-xs transition-colors" style={{ color: liked[i] ? GOLD : "rgba(255,255,255,0.25)" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-white/[0.07] p-5 hover:border-[#F3BA2F]/20 transition-colors">
              ...
              <div className="flex items-center gap-1.5 text-white/35 text-xs font-mono mb-4"><Clock size={10} /><span>Saturday, July 5 · 8:00 PM PKT</span></div>
              <button onClick={() => alert("Zoom link will be sent 30 minutes before the session.")} className="w-full py-2.5 font-bold text-xs tracking-wider font-mono border transition-colors" style={{ borderColor: `${GOLD}40`, color: GOLD }}>JOIN ZOOM SESSION</button>
            </div>
            <div className="border border-white/[0.07] p-5 hover:border-[#F3BA2F]/20 transition-colors">
              <div className="flex items-center gap-2 mb-3">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-1.5 text-white/35 text-xs font-mono"><Mic size={10} /><span>12 listening</span></div>
                <div className="flex items-end gap-0.5 h-4">
                  {[4, 8, 12, 9, 6, 11, 7, 14, 5, 9].map((h, j) => (
                    <motion.div key={j} className="w-1" style={{ height: h, background: GOLD, opacity: 0.6 }} animate={{ height: [h, h * 1.8, h] }} transition={{ duration: 0.5 + (j % 3) * 0.2, repeat: Infinity, delay: j * 0.08 }} />
                  ))}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => alert("Joined as listener.")} className="py-2 border border-white/10 text-white/50 font-mono text-[10px] tracking-wider font-bold hover:border-[#F3BA2F]/30 hover:text-[#F3BA2F] transition-colors">LISTEN</button>
                <button onClick={() => alert("Raise hand sent to host.")} className="py-2 font-mono text-[10px] tracking-wider font-bold text-black transition-colors" style={{ background: GOLD }}>RAISE HAND 🖐</button>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <div className="border border-white/[0.07] p-6">
            <div className="flex flex-wrap gap-6 mb-5">
              <div><div className="font-mono text-[8px] text-white/25 uppercase mb-1">Active Tier</div><div className="font-semibold" style={{ color: cfg.accent }}>{cfg.emoji} {cfg.name}</div></div>
              <div><div className="font-mono text-[8px] text-white/25 uppercase mb-1">Expires</div><div className="text-white/60 text-sm">{new Date(parseInt(LS.get("okz_expires") || String(Date.now() + MS_30_DAYS))).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</div></div>
              <div><div className="font-mono text-[8px] text-white/25 uppercase mb-1">Days Left</div><div className="text-white font-bold">{daysLeft} days</div></div>
            </div>
            <div className="h-0.5 bg-white/[0.06] w-full mb-5">
              <motion.div className="h-full" style={{ background: cfg.accent }} initial={{ width: 0 }} animate={{ width: `${(daysLeft / 30) * 100}%` }} transition={{ duration: 1.2 }} />
            </div>
            <div className="flex flex-wrap gap-2">
              <div className="font-mono text-[8px] text-white/20 flex items-center gap-1.5 mt-1"><Shield size={10} className="text-white/20" />Access auto-revokes upon expiry</div>
            </div>
          </div>
        </motion.div>
      </div>
...
export default function OkzByteHub() {
  ...
  useEffect(() => {
    ...
    if (wallet && tier && expires) {
      ...
      if (Date.now() < exp) { setWalletAddr(wallet); setMemberTier(parseInt(tier) as Tier); setStep("community"); }
      else { setIsExpired(true); setWalletAddr(wallet); setStep("wallet"); }
    }
  }, []);

  const handleWalletConnected = (addr: string) => { setWalletAddr(addr); LS.set("okz_wallet", addr); setStep("tiers"); };
  const handleBuy = (tier: Tier, hash: string) => { setMemberTier(tier); setTxHash(hash); LS.set("okz_tier", String(tier)); LS.set("okz_expires", String(Date.now() + MS_30_DAYS)); LS.set("okz_tx", hash); setStep("card"); };
  const handleCardDone = () => { setStep("community"); };
  const handleExpired = () => { setIsExpired(true); LS.del("okz_tier", "okz_expires", "okz_tx"); setStep("tiers"); };

  return (
    <AnimatePresence mode="wait">
      <motion.div key={step} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.35, ease: [0.25, 0.46, 0.45, 0.94] }}>
        {step === "intro" && <IntroPhase onNext={() => setStep("wallet")} />}
        {step === "wallet" && <WalletPhase onConnected={handleWalletConnected} />}
        {step === "tiers" && <TiersPhase walletAddr={walletAddr} isExpired={isExpired} onBuy={handleBuy} />}
        {step === "card" && <CardPhase tier={memberTier} memberId={memberId} txHash={txHash} onDone={handleCardDone} />}
        {step === "community" && <CommunityPhase tier={memberTier} walletAddr={walletAddr} memberId={memberId} onExpired={handleExpired} />}
      </motion.div>
    </AnimatePresence>
  );
}
